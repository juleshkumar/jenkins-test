            ...
            steps {
                script {
                    dir('julesh-terraform/environments/dev/Ansible') {
                        ...
                        sh "ansible-playbook -i inventory.ini deploy.yml --extra-vars 'efs_dns_name=${env.EFS_DNS_NAME} aws_access_key_id=${env.AWS_ACCESS_KEY_ID} aws_secret_access_key=${env.AWS_SECRET_ACCESS_KEY} aws_region=${params.region} aws_output_format=${params.output} region=${params.region} cluster_name=${params.cluster_name}'"
                }
            }
        }
}
